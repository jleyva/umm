...
function moodleWSCall(method, data, callBack, preSets){
    ...
    $.ajax({
      ...
      success: function(data){
		...
		console.log(data);
		console.log(moodleJSONPrettifier(data));
        callBack(moodleJSONPrettifier(data));
      }
    });    
}
...
function moodleJSONProcessSingle(node){
	...
	for(var i in node.SINGLE[0].KEY){
		var newnode = node.SINGLE[0].KEY[i];
		if(typeof(newnode) == 'object'){
			if(typeof(newnode.VALUE) != 'undefined' )
				singlenode[newnode.name] = newnode.VALUE[0].Text;
			else
...
function moodleJSONPrettifier(json){
	if(typeof(json.MULTIPLE) != 'undefined'){
		...
		for(var i in json.MULTIPLE){
			if(typeof(json.MULTIPLE[i]) == 'object'){
				newjson.push(moodleJSONProcessSingle(json.MULTIPLE[i]));
			}
		}
		return newjson;
	}

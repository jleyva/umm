...
PhoneGap.addConstructor = function(func) {
    PhoneGap.onPhoneGapInit.subscribeOnce(function() {
        try {
            ...
            logInfo("Failed to run constructor: " + e);
        }
    });
};
...
PhoneGap.addPlugin = function(name, obj) {
    if (!window.plugins[name]) {
        ...
        logInfo("Error: Plugin "+name+" already exists.");
    }
};
    ...
    try {
        ...
        if (r.length > 0) {
            ...
            if (v.status === PhoneGap.callbackStatus.OK) {
                ...
                if (success) {
                    try {
                        ...
                        logInfo("Error in success callback: " + callbackId  + " = " + e);
                    }
            ...
            // If error, then display error
                ...
                logInfo("Error: Status="+v.status+" Message="+v.message);
                ...
                if (fail) {
                    try {
                        ...
                        logInfo("Error in error callback: "+callbackId+" = "+e1);
                    }
...
PhoneGap.callbackError = function(callbackId, args) {
    if (PhoneGap.callbacks[callbackId]) {
        try {
            ...
            logInfo("Error in error callback: "+callbackId+" = "+e);
        }
    ...
    xmlhttp.onreadystatechange=function(){
        if(xmlhttp.readyState === 4){
            ...
            if (xmlhttp.status === 200) {
                ...
                setTimeout(function() {
                    try {
                        ...
                        logInfo("JSCallback: Message from Server: " + msg);
                        logInfo("JSCallback Error: "+e);
                    }
                }, 1);
            ...
            // If security error
                ...
                logInfo("JSCallback Error: Invalid token.  Stopping callbacks.");
            }

            // If server is stopping
            else if (xmlhttp.status === 503) {
                logInfo("JSCallback Server Closed: Stopping callbacks.");
            }

            // If request wasn't GET
            else if (xmlhttp.status === 400) {
                logInfo("JSCallback Error: Bad request.  Stopping callbacks.");
            }

            // If error, revert to polling
            else {
                logInfo("JSCallback Error: Request failed.");
                PhoneGap.UsePolling = true;
                PhoneGap.JSCallbackPolling();
            }
        }
    ...
    if (msg) {
        setTimeout(function() {
            try {
                ...
                logInfo("JSCallbackPolling: Message from Server: " + msg);
                logInfo("JSCallbackPolling Error: "+e);
            }
        }, 1);
...
Accelerometer.prototype.getCurrentAcceleration = function(successCallback, errorCallback, options) {
    ...
    if (typeof successCallback !== "function") {
        logInfo("Accelerometer Error: successCallback is not a function");
        return;
    }

    // errorCallback optional
    if (errorCallback && (typeof errorCallback !== "function")) {
        logInfo("Accelerometer Error: errorCallback is not a function");
        return;
    }
...
Battery.prototype._error = function(e) {
    logInfo("Error initializing Battery: " + e);
};
...
Camera.prototype.getPicture = function(successCallback, errorCallback, options) {
    ...
    // successCallback required
    if (typeof successCallback !== "function") {
        logInfo("Camera Error: successCallback is not a function");
        return;
    }

    // errorCallback optional
    if (errorCallback && (typeof errorCallback !== "function")) {
        logInfo("Camera Error: errorCallback is not a function");
        return;
    }
...
Compass.prototype.getCurrentHeading = function(successCallback, errorCallback, options) {
    ...
    // successCallback required
    if (typeof successCallback !== "function") {
        logInfo("Compass Error: successCallback is not a function");
        return;
    }

    // errorCallback optional
    if (errorCallback && (typeof errorCallback !== "function")) {
        logInfo("Compass Error: errorCallback is not a function");
        return;
    }
...
var Device = function() {
    ...
    this.getInfo(
        ...
        function(e) {
            me.available = false;
            logInfo("Error initializing PhoneGap: " + e);
...
Device.prototype.getInfo = function(successCallback, errorCallback) {
    ...
    // successCallback required
    if (typeof successCallback !== "function") {
        logInfo("Device Error: successCallback is not a function");
        return;
    }

    // errorCallback optional
    if (errorCallback && (typeof errorCallback !== "function")) {
        logInfo("Device Error: errorCallback is not a function");
        return;
    }
...
Device.prototype.overrideBackButton = function() {
    logInfo("Device.overrideBackButton() is deprecated.  Use App.overrideBackbutton(true).");
...
Device.prototype.resetBackButton = function() {
    logInfo("Device.resetBackButton() is deprecated.  Use App.overrideBackbutton(false).");
...
Device.prototype.exitApp = function() {
    logInfo("Device.exitApp() is deprecated.  Use App.exitApp().");
...
Geolocation.prototype.getCurrentPosition = function(successCallback, errorCallback, options) {
    if (navigator._geo.listeners.global) {
        logInfo("Geolocation Error: Still waiting for previous getCurrentPosition() request.");
        try {
...
Geolocation.prototype.success = function(id, lat, lng, alt, altacc, head, vel, stamp) {
    ...
    try {
        ...
        logInfo("Geolocation Error: Error calling success callback function.");
    }
...
Geolocation.prototype.fail = function(id, code, msg) {
    try {
        ...
        logInfo("Geolocation Error: Error calling error callback function.");
    }
};
...
var Media = function(src, successCallback, errorCallback, statusCallback, positionCallback) {
    ...
    if (successCallback && (typeof successCallback !== "function")) {
        logInfo("Media Error: successCallback is not a function");
        return;
    }

    // errorCallback optional
    if (errorCallback && (typeof errorCallback !== "function")) {
        logInfo("Media Error: errorCallback is not a function");
        return;
    }

    // statusCallback optional
    if (statusCallback && (typeof statusCallback !== "function")) {
        logInfo("Media Error: statusCallback is not a function");
        return;
    }

    // statusCallback optional
    if (positionCallback && (typeof positionCallback !== "function")) {
        logInfo("Media Error: positionCallback is not a function");
        return;
    }
...
var Connection = function() {
    ...
    this.getInfo(
        ...
        function(e) {
            ...
            logInfo("Error initializing Network Connection: " + e);
        });
};
        ...
        try {
            ...
            if (tx && tx.queryList[id]) {
                ...
                try {
                    ...
                    logInfo("executeSql error calling user success callback: "+ex);
                }

                tx.queryComplete(id);
            }
        } catch (e) {
            logInfo("executeSql error: "+e);
        }
    }
};
        ...
        try {
            ...
            if (tx && tx.queryList[id]) {
                ...
                try {
                    ...
                    logInfo("executeSql error calling user error callback: "+ex);
                }
    ...
    if (this.successCallback) {
        ...
        if (count === 0) {
            try {
                ...
                logInfo("Transaction error calling user success callback: " + e);
            }
        }
    }
};
...
DroidDB_Tx.prototype.queryFailed = function(id, reason) {
    ...
    if (this.errorCallback) {
        try {
            ...
            logInfo("Transaction error calling user error callback: " + e);
        }
    }
};
